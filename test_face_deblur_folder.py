...
def ensure_directory_exists(path):
  if not os.path.exists(path):  # 检查文件夹是否存在
    os.makedirs(path)  # 如果不存在，创建文件夹
    print(f"Directory created: {path}")


out_root = "/home/dejavu/Code/UMSN-Face-Deblurring/mediapipe_walking_deblur"

folder_root = "/home/dejavu/Code/UMSN-Face-Deblurring/mediapipe_walking_folder"
foldername_list = os.listdir("/home/dejavu/Code/UMSN-Face-Deblurring/mediapipe_walking_folder")
folder_list_path = []
for path in foldername_list:
  folder_list_path.append(os.path.join(folder_root, path))
for mat_path in folder_list_path:
  folder_name = mat_path.split('/')[-1]
      ...
      if 1:
        ...
        with torch.no_grad():
          for idx in range(val_input.size(0)):
              ...
              filename = os.path.join(out_dir, f'{i+1:06d}.png')
